    ...
    <div class="container mt-5">
        ...
        <form action="../controllers/controlerServico.php" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="nome" name="nome" value="<?= $servico ? $servico->getNome() : ''; ?>" required>
            </div>
            
            <div class="mb-3">
                <label for="descricao" class="form-label">Descrição</label>
                <textarea class="form-control" id="descricao" name="descricao" rows="3" required><?= $servico ? $servico->getDescricao() : ''; ?></textarea>
            </div>
            
            <div class="mb-3">
                <label for="preco_base" class="form-label">Preço Base</label>
                <input type="number" step="0.01" class="form-control" id="preco_base" name="preco_base" value="<?= $servico ? $servico->getPrecoBase() : ''; ?>" required>
            </div>
            
            <div class="mb-3">
                <label for="imagem_url" class="form-label">URL da Imagem</label>
                <input type="text" class="form-control" id="imagem_url" name="imagem_url" value="<?= $servico ? $servico->getImagemUrl() : ''; ?>">
            </div>
            
            <div class="mb-3">
                <label for="tipo_servico" class="form-label">Tipo de Serviço</label>
                <input type="text" class="form-control" id="tipo_servico" name="tipo_servico" value="<?= $servico ? $servico->getTipoServico() : ''; ?>" required>
            </div>
